...
string cfg = "";
    ifstream cfgfile("C:/Users/Public/Documents/Breadbox IDE/emucfg.txt");
    if (!cfgfile) {
        cout << "cfg file error";
        return 0;
    }
    ...
    cfg = buffer.str();
int firstDash = char_index('-',1,cfg);
int secondDash = char_index('-',2,cfg);
int thirdDash = char_index('-',3,cfg);

showvar = stoi(trim(cfg.substr(0, firstDash)));
delay = stoi(trim(cfg.substr(firstDash + 1, secondDash - firstDash - 1)));
ctf = stoi(trim(cfg.substr(secondDash + 1, thirdDash - secondDash - 1)));
langin = stoi(trim(cfg.substr(thirdDash + 1)));

    }



    ifstream file("C:/Users/Public/Documents/Breadbox IDE/temp.txt");
    if (!file) {
        cout << "temp file error";
        return 0;
    }
    ...
    emulating = true;
    while (emulating){
    if (showvar != 99) {cout << registers[showvar] << '\n';}
    do_function();
    addr++;
