...
int main() {
    ...
    int run;
    cin >> run;
    if (run == 0){select_file = false;actioning = false;}
    else if (run == 1){actioning = false;}
    ...
    emulating = true;
    while (emulating){
    //cout << registers[0] << '\n';  //REMOVE THE // FROM THE BEGGINING OF THIS TO DISPLAY REG A0 FOR THE EXAMPLE PROGRAM
    do_function();
    addr++;
